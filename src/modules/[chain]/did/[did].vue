...
const props = defineProps(['did', 'chain']);
...
const didres = ref({} as Didresponce);
...
onMounted(() => {
  console.log("Address", props.did)
  loadAccount(props.did);
});
...
const totalValue = computed(() => {
 ...
 async function loadAccount(did: string) {
  const address = await blockchain.rpc.getDid( did ).then((x) => {
    didres.value = x.id
    return x?.id.creator
  })
  ...
  <div v-if="account">
    ...
    <div class="bg-base-100 px-4 pt-3 pb-4 rounded mb-4 shadow">
      <div class="flex items-center mb-3">
        <!-- img -->
        <div class="inline-flex relative w-5 h-5 rounded-md">
          <div
            class="w-5 h-5 absolute rounded-md opacity-10 bg-primary"
          ></div>
          <div
        ...
        <div class="flex flex-1 flex-col truncate pl-4">
          <h2 class="text-sm card-title">{{ $t('account.did') }}:</h2>
          <span class="text-xs truncate"> {{ didres.did }}</span>
        </div>
      </div>
      <div class="flex items-center mb-3">
        <!-- img -->
        ...
        <div class="flex flex-1 flex-col truncate pl-4">
          ...
          <span class="text-xs truncate"> {{ didres.creator }}</span>
        </div>
      </div>
      ...
      <div class="flex items-center">
        ...
        <!-- content -->
        <div class="flex flex-1 flex-col truncate pl-4">
          <h2 class="text-sm card-title">{{ $t('account.username') }}:</h2>
          <span class="text-xs truncate"> {{ didres.username }}</span>
        </div>
      </div>
    </div>
